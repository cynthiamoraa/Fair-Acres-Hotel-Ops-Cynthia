...
export default function SettingsPage({ workers, onWorkersChange }) {
  ...
  async function handleChangePassword(e) {
    e.preventDefault();
    setPwMsg(null);
    const res = await postJson("/auth/manager/change-password", { currentPassword, newPassword });
    const data = await res.json();
    ...
    <section className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
        ...
        <form onSubmit={handleSaveSettings} className="space-y-3">
          ...
          <div>
            ...
            <input type="email" className="mt-1 h-11 w-full rounded-xl border border-slate-200 px-4 text-sm outline-none focus:border-slate-400"
              value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} />
          </div>
          <div className="flex items-center gap-3">
            <button className="h-11 px-6 rounded-xl bg-[#111827] text-white text-sm font-semibold hover:bg-slate-800">Save Changes</button>
            {settingsSaved && <p className="text-sm text-emerald-600 font-medium">Saved ✓</p>}
          </div>
        </form>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
        ...
        <form onSubmit={handleChangePassword} className="space-y-3">
          <input type="password" className="h-11 w-full rounded-xl border border-slate-200 px-4 text-sm outline-none focus:border-slate-400"
            placeholder="Current password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} required />
          <input type="password" className="h-11 w-full rounded-xl border border-slate-200 px-4 text-sm outline-none focus:border-slate-400"
            placeholder="New password (min 6 chars)" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required />
          {pwMsg && <p className={`text-sm font-medium ${pwMsg.ok ? "text-emerald-600" : "text-rose-600"}`}>{pwMsg.text}</p>}
          <button className="h-11 px-6 rounded-xl bg-[#111827] text-white text-sm font-semibold hover:bg-slate-800">Change Password</button>
        </form>
      </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => { setChangingPinFor(changingPinFor === w.id ? null : w.id); setNewPin(""); }}
                    className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-200 hover:text-slate-700" title="Change PIN">
                    <KeyRound size={15} />
                  </button>
              ...
              {changingPinFor === w.id && (
                <div className="flex gap-2 pt-1">
                  <input type="password" inputMode="numeric" maxLength={8}
                    ...
                    placeholder="New PIN (min 4 digits)" value={newPin} onChange={(e) => setNewPin(e.target.value)} />
                  <button onClick={() => handleChangePin(w.id)}
        ...
        <form onSubmit={handleAddWorker} className="space-y-2">
          ...
          <div className="relative">
            <input type={showNewPin ? "text" : "password"} inputMode="numeric" maxLength={8}
              ...
              placeholder="Set PIN (min 4 digits)" value={newWorkerPin} onChange={(e) => setNewWorkerPin(e.target.value)} />
